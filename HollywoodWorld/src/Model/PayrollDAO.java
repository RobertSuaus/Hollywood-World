...
public class PayrollDAO extends BaseDAO {
    ...
    public static int save(PayrollBreakdown desglose ){
        ...
        String sql = "INSERT INTO desglose(desgloseId, employeeId, ISR"+
            ...
            "primaDominical, diasTrabajados, totalPago, salarioIntegro, "+
            "fechaDesglose)VALUES($desgloseId$, $employeeId$, $ISR$, "+
            "$seguro$, $aguinaldo$, $vacaciones$, "+
            "$primaDominical$, $diasTrabajados$, $totalPago$, $salarioIntegro$, '$fechaDesglose$')";
        ...
        sql = sql.replace("$seguro$", String.valueOf(desglose.getRetention().getTaxPerIMSS()));
        ...
        sql = sql.replace("$totalPago$", String.valueOf(desglose.getTotalPayment()) );
        sql = sql.replace("$salarioIntegro$", String.valueOf(desglose.getIntegratedWage()) );
        
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
        
        sql = sql.replace("$fechaDesglose$", dateFormat.format(desglose.getBreakdownDate() ) );
    ...
    public static String[] getBreakdownSummary(Date startDate, Date endDate){
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
        ...
        try{
            ...
            registry[4] = String.valueOf(resultSet.getDouble(5));
            registry[5] = String.valueOf(resultSet.getInt(6));
    ...
    public static ArrayList<PayrollBreakdown> getBreakdownList(int employeeId) throws ParseException{
           ...
           while(resultSet.next()){
               PayrollBreakdown payrollBreakdown = new PayrollBreakdown(
                   ...
                   new Bonus(
                       ...
                       resultSet.getDouble("vacaciones"),
